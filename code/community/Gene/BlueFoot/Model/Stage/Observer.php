...
class Gene_BlueFoot_Model_Stage_Observer
{
    const GENE_BLUEFOOT_HEAD = 'BLUEFOOT_HEAD';
    ...
    /**
     * Store the head block
     *
     * @var bool|Mage_Page_Block_Html_Head
     */
    private $_headBlock = false;
        ...
        if($response instanceof Mage_Core_Controller_Response_Http) {
            ...
            if (!empty($html)) {
                ...
                if (strpos($html, Gene_BlueFoot_Model_Stage_Save::GENE_CMS_STRING) !== false) {
                    ...
                    // Render the page
                    $renderedPage = $render->render($html, $this->_headBlock);
                    ...
                    // Do we need to re-insert the head block?
                    if ($this->_headBlock instanceof Mage_Page_Block_Html_Head) {
                        // Restore the head if nothing needs to be rendered
                        $response->setBody($render->replaceHeadHtml($html, $this->_headBlock->toHtml()));
                    } else {
    ...
    /**
     * Event called on block to HTML after
     *
     * @param \Varien_Event_Observer $observer
     *
     * @return $this
    ...
    public function toHtmlAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($this->_headBlock == false && $block instanceof Mage_Page_Block_Html_Head) {
            $this->_headBlock = clone $block;
            $transport->setHtml('<!--' . self::GENE_BLUEFOOT_HEAD . '-->');
        }
