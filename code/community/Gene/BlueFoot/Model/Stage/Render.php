...
class Gene_BlueFoot_Model_Stage_Render extends Mage_Core_Model_Abstract
{
     ...
     *
     * @param $html
     * @param $headBlock Mage_Page_Block_Html_Head
     *
     * @return mixed
     */
    public function render($html, $headBlock)
    {
        ...
        if(!empty($pageBuilderSections)) {
            ...
            // Do we need to load assets into the head
            if ($headBlock instanceof Mage_Page_Block_Html_Head) {
                $renderedHtml = $this->loadAssets($headBlock, $renderedHtml);
            }
        ...
        // Is the head block present?
        if ($headBlock instanceof Mage_Page_Block_Html_Head) {
            // We need to restore the original head
            return $this->replaceHeadHtml($html, $headBlock->toHtml());
        }
    ...
    /**
     * Find any assets associated with content blocks, and append them into the head
     *
     ...
     * @return mixed
     */
    public function loadAssets($head, $renderedHtml)
    {
        if (!empty($this->_contentTypes)) {

            // Try and load the head from the cache
            if (Mage::app()->useCache('gene_block_cache') && false !== ($headHtml = Mage::app()->getCacheInstance()->load($this->getAssetCacheKey($this->_contentTypes)))) {
                return $this->replaceHeadHtml($renderedHtml, $headHtml);
            }

            $contentTypes = array_unique($this->_contentTypes);
            foreach ($contentTypes as $contentType) {
                $type = $this->_loadedTypes->getItemByColumnValue('identifier', $contentType);
                if ($type && $type->getId()) {
                    if ($itemViewTemplate = $type->getItemViewTemplate()) {
                        /* @var $assets Mage_Core_Model_Config_Element */
                        $assets = Mage::helper('gene_bluefoot/config')->getConfig('content_blocks/templates/' . $itemViewTemplate . '/assets');
                        if ($assets !== false) {
                            foreach ($assets->asArray() as $asset) {
                                switch ($asset['type']) {
                                    case 'css':
                                        $head->addCss($asset['name']);
                                        break;
                                    case 'js':
                                        $head->addJs($asset['name']);
                                        break;
                                    case 'skin_js':
                                        $head->addItem('skin_js', $asset['name']);
                                        break;
                                    case 'js_css':
                                        $head->addItem('js_css', $asset['name']);
                                        break;
                                }
                            }
                        }
            ...
            if(Mage::app()->useCache('gene_block_cache')) {
                Mage::app()->getCacheInstance()->save($head->toHtml(), $this->getAssetCacheKey($this->_contentTypes), Mage::app()->getCacheInstance()->getTagsByType('gene_block_cache'), 604800);
            }

            return $this->replaceHeadHtml($renderedHtml, $head->toHtml());
        }
    }

    /**
     * Place the newly generated head into the rendered HTML
     *
     * @param $renderedHtml
     * @param $headHtml
     *
     * @return mixed
     */
    public function replaceHeadHtml($renderedHtml, $headHtml)
    {
        // Restore the head block back into the output
        return str_replace('<!--' . Gene_BlueFoot_Model_Stage_Observer::GENE_BLUEFOOT_HEAD . '-->', $headHtml, $renderedHtml);
    }
